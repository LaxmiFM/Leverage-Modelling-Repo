...
def top5LargestIndustries(df_Portfolio1, df_ExcessConcentration1):
    df_Portfolio1['Borrower_largestIndustry'], df_Portfolio1['Excess_EQ_largestIndustry'] = zip(
        *df_Portfolio1.apply(lambda x: returnExcess(x['Borrower'], x['Excess Largest Industry'], x['Largest Industry']),
                             axis=1))
    df = df_Portfolio1[['Borrower_largestIndustry', 'Excess_EQ_largestIndustry', 'Largest Industry']].copy()
    ...
    df = df.rename(
        columns={'Borrower_largestIndustry': 'Borrower', 'Excess_EQ_largestIndustry': 'Excess for the Industry',
                 'Largest Industry': 'Rank'})
    df.reset_index(inplace=True)
    df['Excess Concentration Limit Percentage'] = df["Rank"].apply(
        lambda x: (x == 1 and df_ExcessConcentration1.iloc[3]['Concentration Limit Percentage']) or (
                x == 2 and df_ExcessConcentration1.iloc[4]['Concentration Limit Percentage']) or (
                          x == 3 and df_ExcessConcentration1.iloc[5]['Concentration Limit Percentage']) or
                  df_ExcessConcentration1.iloc[6]['Concentration Limit Percentage'])
    df['Concentration Limit Amount'] = df["Rank"].apply(
        lambda x: (x == 1 and df_ExcessConcentration1.iloc[3]['Concentration Limit Values']) or (
                x == 2 and df_ExcessConcentration1.iloc[4]['Concentration Limit Values']) or (
                          x == 3 and df_ExcessConcentration1.iloc[5]['Concentration Limit Values']) or
                  df_ExcessConcentration1.iloc[6]['Concentration Limit Values'])
    df['Applicable Test Limit'] = df["Rank"].apply(
        lambda x: (x == 1 and df_ExcessConcentration1.iloc[3]['Applicable Test Limit']) or (
                x == 2 and df_ExcessConcentration1.iloc[4]['Applicable Test Limit']) or (
                          x == 3 and df_ExcessConcentration1.iloc[5]['Applicable Test Limit']) or
                  df_ExcessConcentration1.iloc[6]['Applicable Test Limit'])
...
def Top5LargestExcess(df_Portfolio1,df_ExcessConcentration1):
    df_Portfolio1['Borrower_Rank'], df_Portfolio1['Largest_Excess_Rank'] = zip(
        *df_Portfolio1.apply(lambda x: returnExcessObligor(x['Borrower'], x['Rank EJ'], x['Excess Largest Obligor']),
                             axis=1))
    df = df_Portfolio1[['Borrower_Rank', 'Largest_Excess_Rank','Rank EJ']].copy()
    ...
    df = df.rename(columns={'Borrower_Rank': 'Borrower', 'Largest_Excess_Rank': 'Largest Excess Amount','Rank EJ':'Rank'})
    df.reset_index(drop=True, inplace=True)
    df['Excess Concentration Limit Percentage'] = df["Rank"].apply(
        lambda x: (df_ExcessConcentration1.iloc[2]['Concentration Limit Percentage']))
    df['Concentration Limit Amount'] = df["Rank"].apply(
        lambda x: (df_ExcessConcentration1.iloc[2]['Concentration Limit Values']))
    df['Applicable Test Limit'] = df["Rank"].apply(
        lambda x: (df_ExcessConcentration1.iloc[2]['Applicable Test Limit']))
    res = sum(df['Largest Excess Amount'])
